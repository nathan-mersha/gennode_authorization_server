 ...
 * @module              app.js
 * @description         Starting point for hisab_authorization_service service.
 * @kind                Init
...
let
    ...
    debug       = require('debug')('hisab_authorization_service'),
...
app.use(function (req, res, next) {
    if(req.url === "/") {
        res.redirect(`${config.REVERSE_PROXY}:${config.HTTP_PORT}//auth/apidoc`);
        res.status(200);
    }else{
